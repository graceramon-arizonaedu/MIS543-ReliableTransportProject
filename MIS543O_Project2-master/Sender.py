...
class Sender(BasicSender.BasicSender):
    ...
    def start(self):
        ...
        while not msg_type == 'end':
            ...
            #corruption
            if Checksum.validate_checksum(response) == False:
                self.send(packet)
            
            #duplication
            #msg_type, ackno, data, checksum = self.split_packet(response)
            ackno = self.split_packet(response)[1]
            if ackno != seqno+1:
            #if response_packet[1] != response_packet[0]+1:
                self.send(packet)
            ...
            #delay 
            #put timeout in receive function 
